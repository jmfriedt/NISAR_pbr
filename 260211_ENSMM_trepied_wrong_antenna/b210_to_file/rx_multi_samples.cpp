...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    desc.add_options()
        ...
        ("gain", po::value<double>(&gain)->default_value(55), "gain for the RF chain")
...
//        usrp->set_rx_antenna(ant, channel);
    ...
    uhd::stream_args_t stream_args("sc16","sc16"); // JMF  sc8-sc16
    ...
    std::vector<std::vector<std::complex<short>>> buffs(                                 // char-short
        usrp->get_rx_num_channels(), std::vector<std::complex<short>>(samps_per_buff));  // char-short

    // create a vector of pointers to point to each of the channel buffers
    std::vector<std::complex<short>*> buff_ptrs;                                         // char-short
...
for (int jmf=0;jmf<6;jmf++)   // 8 GB RPi4
   {num_acc_samps = 0; // number of accumulated samples
    while (num_acc_samps < total_num_samps) {
        ...
        outfile1.write( (const char*)&buffs[0].front(), samps_per_buff*sizeof(std::complex<short>));  // char-short
        outfile2.write( (const char*)&buffs[1].front(), samps_per_buff*sizeof(std::complex<short>));  // char-short
