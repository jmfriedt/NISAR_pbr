...
beta_I = linspace(0,35000,3201);nbe = length(beta_I);
F1 = exp(-1j*2*pi*freq*beta_I/c)/sqrt(Nr);

al_I = linspace(-7e-3,7e-3,2601);nal = length(al_I);
...
xm = linspace(0,20e3,4001);nxm = length(xm);
ym = linspace(-6000,6000,3001);nym = length(ym);
