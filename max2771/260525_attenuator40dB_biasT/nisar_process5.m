...
%% Load data
f12=fopen('cut.bin');  % ref
fseek(f12,fs*10);     % packed char = 1x
...
al_I = linspace(-7e-3,7e-3,2501);nal = length(al_I);
...
colormap(jet);colorbar;clim([-22,0]);
...
ym = linspace(-4000,6000,3001);nym = length(ym);
...
clim([-22,0]);colormap(jet);colorbar;
