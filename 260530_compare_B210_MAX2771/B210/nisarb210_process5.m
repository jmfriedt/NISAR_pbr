...
%% Load data
f1=fopen('ref.bin');  % 2=ref
fseek(f1,13*fs*2*2);  % for searching the chirp shape (*2 complex, *2 16bits)
f2=fopen('sur.bin');  % 1=sur
fseek(f2,13*fs*2*2);  % for searching the chirp shape (*2 complex, *2 16bits)
clear d
...
colormap(jet);colorbar;clim([-56,0]);
...
clim([-56,0]);colormap(jet);colorbar;
