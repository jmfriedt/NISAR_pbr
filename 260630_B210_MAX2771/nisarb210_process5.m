...
%% Load data
f1=fopen('b210_2ref.bin');  % 2=ref
f2=fopen('b210_1sur.bin');  % 1=sur
clear d
...
pindx=kpos(7000:7000+P+L);
 ...
 for p = 1:P
     disp(p);
     %ref1(pindx(p)-AS:pindx(p)-AS+Nr-1)=ref1(pindx(p)-AS:pindx(p)-AS+Nr-1)-mean(ref1(pindx(p)-AS:pindx(p)-AS+Nr-1));
     %sur1(pindx(p)-AS:pindx(p)-AS+Nr-1)=sur1(pindx(p)-AS:pindx(p)-AS+Nr-1)-mean(sur1(pindx(p)-AS:pindx(p)-AS+Nr-1));
